function varargout = TugasPCD_8_1301164036_MuhammadAlfisarRAchman(varargin)

gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @TugasPCD_8_1301164036_MuhammadAlfisarRAchman_OpeningFcn, ...
                   'gui_OutputFcn',  @TugasPCD_8_1301164036_MuhammadAlfisarRAchman_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
% --- Executes just before TugasPCD_8_1301164036_MuhammadAlfisarRAchman is made visible.
function TugasPCD_8_1301164036_MuhammadAlfisarRAchman_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line output for TugasPCD_8_1301164036_MuhammadAlfisarRAchman
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes TugasPCD_8_1301164036_MuhammadAlfisarRAchman wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = TugasPCD_8_1301164036_MuhammadAlfisarRAchman_OutputFcn(hObject, eventdata, handles)
...
img = handles.image1; % Mendapatkan Data image yang telah dimasukan ke dalam data handles
img = uint8(img); % merubah isi dari matriks img menjadi uint8
[r, c, colormap] = size(img); % Mengisi nilai r,c,dan colormap dengan size yang ada di dalam img

for i=1 : r %Menjelajahi pixel row
    for j=1 : c %Menjelajahi pixel colom
        if mod(img(i,j,:),5) == 0 % pengecekan apakah isi dari pixel tersebut di mod 5 hasilnya adalah 0
            img(i,j,:) = img(i,j,:)/5; % Mengisi pixel tersebut dengan pixel tersebut di bagi 5
%             disp(img(i,j,:))
        else
            img(i,j,:) = int8(img(i,j,:)/5)+1; % Mengisi pixel tersebut dengan pixel tersebut di bagi 5 terus di tambah 1
        end
    end
end
imwrite(img, 'hasil kompresi.jpg');
figure,imshow(img);
